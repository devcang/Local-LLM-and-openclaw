...
下载Linux的，注意区分 GPU 和 CPU 

如下载的是CPU版本，则解压到 /opt/CLAW/llama_cpu 中，即 /opt/CLAWllama_cpu/llama-server 可直接执行。后面以此路径说明。  

#### 添加到 PATH,修改个人目录的.bashrc，如 nano ~/.bashrc，后边添加 <br/>
export PATH=$PATH:/opt/CLAW/llama_cpu   

## ............................下载大模型、运行

## 1. 到llama.cpp目录
cd /opt/CLAW/llama_cpu  
## 2. 创建模型目录
mkdir -p models  
...
/opt/CLAW/llama_cpu/llama-cli -m /opt/CLAW/llama_cpu/models/qwen3.5-0.8b-q4_k_m.gguf -t 6 -cnv -p "你是一个有用的AI助手，请用中文回答。"
...
#### 步骤 3：配置运行环境
将 ZLUDA 提供的 nvcuda.a 和 nvml.a 复制到你要运行的应用程序所在目录。  
或者使用 ZLUDA 提供的启动器： <ZLUDA_DIRECTORY>zluda_with -- <APPLICATION> <<ARGUMENTS>>
...
## ............................  Git  
需要 git  
对于 Ubuntu 用户，此 PPA 提供了最新的上游 Git 稳定版本：
# add-apt-repository ppa:git-core/ppa
# apt update; apt install git

## ............................ 安装 Node.js
sudo apt install -y curl ca-certificates gnupg
https://www.yaoyouai.cn/blog/232/ubuntu-24-install-nodejs/
curl -fsSL https://deb.nodesource.com/setup_24.x | sudo -E bash -

sudo apt search nodejs  | grep node.js
...
## ............................ 安装 OpenClaw 
### 准备用户
在 Ubuntu 通常不使用root，而是在安装Ubuntu时，会创建一个用户。  
所以，假设用户名是 admin  
##### 如果要创建用户，则如
###### 添加用户
...
##### 如果想添加 openclaw 特定的组如claw，则如
sudo groupadd claw
sudo usermod -a -G claw admin

## 正式安装
#### 1、命令行执行    
cd /home/admin/ && mkdir && /home/admin/claw  
安装最新版本  
npm i openclaw  
安装指定版本  
...
#### 2、查看是否安装成功
/home/admin/claw/node_modules/.bin/openclaw --version

<p></p>
<p>
#### 添加到 PATH,修改个人目录的.bashrc，如 nano ~/.bashrc，后边添加 <br/>
export PATH=$PATH:/home/admin/claw/node_modules/.bin   
</p>
...
export llamaPath=/opt/CLAW/llama_cpu/llama-server  
export modelPath=/opt/CLAW/llama_cpu/models/qwen3.5-0.8b-q4_k_m.gguf  
export net_info=" --host 127.0.0.1 --port 8000"
$llamaPath -m $modelPath $net_info --api-key sk-local -c 16384 -b 512 -ngl 0 --threads 4

### 参数说明
#### 1. -m /opt/CLAW/llama_cpu/models/qwen3.5-0.8b-q4_k_m.gguf
全称：--model
作用：指定要加载的模型文件路径
...
## ............................  配置 OpenClaw 对接本地模型
\# 执行命令  
/home/admin/claw/node_modules/.bin/openclaw onboard
配置选项（按顺序选）
...
#### 5、启动 OpenClaw 并测试
\# 命令行启动 OpenClaw 网关  
/home/admin/claw/node_modules/.bin/openclaw gateway --verbose  

\# 前台运行 --- 日常测试维护，运行这个命令就可以  
...
\# 主目录，默认值:~/.openclaw/  
export OPENCLAW_HOME=`pwd`/openclaw_moved  
  
\# state文件所在目录，默认值:~/.openclaw/  
export OPENCLAW_STATE_DIR=`pwd`/openclaw_moved  
  
\# 配置文件全路径，默认值:~/.openclaw/openclaw.json  
export OPENCLAW_CONFIG_PATH=`pwd`/openclaw_moved/openclaw.json  

\# log级别，2个可选:debug trace  
...
#### 1. 安装 QQ 机器人插件
/home/admin/claw/node_modules/.bin/openclaw plugins install @sliverp/qqbot@latest

#### 2. 配置机器人 Token（AppID:AppSecret 格式）
/home/admin/claw/node_modules/.bin/openclaw channels add --channel qqbot --token "102917561:你的AppSecret"

#### 3. 重启 Gateway 让配置生效
/home/admin/claw/node_modules/.bin/openclaw gateway restart
